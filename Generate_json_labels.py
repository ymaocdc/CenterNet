...
if __name__ == "__main__":
    ...
    for id in tqdm(range(len(train))):#len(train)):
        ...
        for car_model, yaw, pitch, roll, x, y, z in zip(model_types, yaws, pitches, rolls, xs, ys, zs):
            ...
            # opencv 3.2
            # mask_new, contours, hierarchy = cv2.findContours((overlay).astype(np.uint8), cv2.RETR_TREE,
            #                                                  cv2.CHAIN_APPROX_SIMPLE)
            # before opencv 3.2
            # contours, hierarchy = cv2.findContours((overlay).astype(np.uint8), cv2.RETR_TREE,
            #                                                    cv2.CHAIN_APPROX_SIMPLE)
            # segmentation = []
