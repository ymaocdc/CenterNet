...
from src.lib.utils.ddd_utils import ddd2locrot
...
if __name__ == "__main__":
    ...
    # k is camera instrinsic matrix
    calib = np.array([[2304.5479, 0, 1686.2379, 0],
    ...
    for id in tqdm(range(len(train))):#len(train)):
        ...
        for car_model, yaw, pitch, roll, x, y, z in zip(model_types, yaws, pitches, rolls, xs, ys, zs):
            ...
            img_cor_points[0] /= img_cor_points[2]
            img_cor_points[1] /= img_cor_points[2]
            cv2.circle(image, (int(img_cor_points[0]), int(img_cor_points[1])), 5, (255, 255, 255), -1)
            ...
            cv2.rectangle(image, (obj['2D_bbox_xyxy'][0], obj['2D_bbox_xyxy'][1]),
                          (obj['2D_bbox_xyxy'][2], obj['2D_bbox_xyxy'][3]), (255, 0,0) , 5)
            obj['local_yaw'] = local_yaw
            obj['theta'] = theta
            font = cv2.FONT_HERSHEY_SIMPLEX
            text = '{} {}'.format(int(np.rad2deg(local_yaw)), int(np.rad2deg(obj['global_yaw'])))
            cv2.putText(image, text, tuple((obj['2D_bbox_xyxy'][0], obj['2D_bbox_xyxy'][1] - 5)), font, 1,
        ...
        rot_y = obj['global_yaw']-np.pi/2
        # k is camera instrinsic matrix
        calib = np.array([[2304.5479, 0, 1686.2379, 0],
                      [0, 2305.8757, 1354.9849, 0],
                      [0, 0, 1, 0]], dtype=np.float32)
        box_3d = compute_box_3d(obj['3D_dimension'], obj['3D_location'][0:3], rot_y)
        box_2d = project_to_image(box_3d, calib)
        cl = (0,255,255)
        img = draw_box_3d(img, box_2d, cl)
        plt.figure(figsize=(20, 20))
