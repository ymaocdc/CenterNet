...
class Box(object):
    ...
    def lift_to_3d(self, mode='optimization'):
        ...
        if self.valid.sum() >=2:
            ...
            if self.bpe_l < self.bpe_r:
                self.local_yaw = self.get_local_yaw_when_far(self.xmin, self.xmax, self.bpe_l, self.bpe_r, aspect_ratio=2.5)

            else:
                self.local_yaw = self.get_local_yaw_when_far(self.xmin, self.xmax, self.fpe_r, self.fpe_l, aspect_ratio=2.5) + np.pi
    ...
    def translation_constraints_try_error_yawLWH(self, try_yaw, try_L, try_W, try_H):
        ...
        if try_L:
            try_L_range = np.arange(3.0, 5.4, 0.2)
        else:
            try_L_range = [self.L]

        if try_W:
            try_W_range = np.arange(1.8,2.3,1)
        else:
             try_W_range = [self.W]

        if try_H:
            # TODO: add car subtype. Go up to 4 only for trucks
            try_H_range = np.arange(1.2, 1.8, 0.1)
        else:
            try_H_range = [self.H]
    ...
    def map_to_2d(X ,Y, Z, L, W, H, yaw, pitch, roll, offset_from_center=None,K=np.array([[2304.5479, 0, 1686.2379],
        ...
        t = np.array([X ,Y, Z])
        Rt[:3, 3] = t
        Rt[:3, :3] = Box.euler_to_Rot(yaw, pitch, roll).T
        Rt = Rt[:3, :]
    ...
    def get_direction_visability(self):
        ...
        if self.direction == 'same':
            if self.valid[0] and self.valid[1]:#two bpe exists
                if abs(self.bpe_l-self.xmin) < abs(self.bpe_r - self.xmax):#forward right
                    self.bpe_l = self.xmin
                    self.vis = 1
                    self.xmax = self.fpe_r
                else:#forward left
                    self.bpe_r = self.xmax
                    self.vis = 0
                    self.xmin = self.fpe_l
            elif self.valid[0]:
                self.vis = 0
            ...
            if self.valid[2] and self.valid[3]:
                ...
                if self.fpe_l < self.xmin:
                    self.xmin = min(self.fpe_r, self.fpe_l)
                    self.vis = 1
                elif self.fpe_r > self.xmax:
                    self.xmax = max(self.fpe_l, self.fpe_r)
                    self.vis = 0
        # if self.bpe_l < self.bpe_r:
        #     self.direction = 'same'
        #     if self.valid[0] and self.valid[1]:#two bpe exists
