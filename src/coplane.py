...
def demo(opt):
    ...
    if opt.demo == 'webcam' or \
        ...
        for (image_name) in image_names:
            ...
            # if not 'ID_0f1f9d5e4' in image_name:
            #     continue
            if debug:
            ...
            with open(os.path.join(model_outputfolder, image_name.split('/')[-1].split('.')[0] + '.json'),
                      'w') as f_out:
            ...
            try:
                ...
                sol = leastsq(residuals, p0, args=(None, XYZ.T))[0]

                # print("Solution: ", sol)
                # print("Old Error: ", (f_min(XYZ.T, p0) ** 2).sum())
                # print("New Error: ", (f_min(XYZ.T, sol) ** 2).sum())
                ...
                for cls_ind in ret:
                    for j in range(len(ret[cls_ind])):
                        ...
                        fit_z = -d/((cx-px)/fx*a + (cy-py)/fy*b + c)
                        fit_x, fit_y = fit_z*(cx-px)/fx, fit_z*(cy-py)/fy
                        ret[cls_ind][j, 8:11] = fit_x, fit_y, fit_z
            except:
                print('fitting error')
            ...
            for cls_ind in ret:
                for j in range(len(ret[cls_ind])):
                    ...
                    try:
                        box3d = Box(xmin, ymin, xmax, ymax, bpe_l, bpe_r, fpe_l, fpe_r, calib, L, W, H, tx, ty, tz,
                                    pitch, center3d=center3d, roll=roll)
                        box3d.lift_to_3d()
                        if debug:
            ...
            predictions[image_name.split('/')[-1].split('.j')[0]] = ' '.join(
                predictions[image_name.split('/')[-1].split('.j')[0]])
