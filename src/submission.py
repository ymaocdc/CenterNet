...
def demo(opt):
  ...
  outputfolder = os.path.join(opt.root_dir, 'resutls')
  ...
  if opt.demo == 'webcam' or \
    ...
    test.to_csv(os.path.join(opt.root_dir, 'submission_pku_everything_equalweight.csv'), index=False)
    test.head()
    print(os.path.join(opt.root_dir, 'submission_pku_everything_equalweight.csv'))
    # for (image_name) in image_names:
    #   ret = detector.run(image_name)
    #   time_str = ''
    #   for stat in time_stats:
