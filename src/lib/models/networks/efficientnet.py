...
from .resnet_dcn import fill_fc_weights, fill_up_weights
...
class EffNet(nn.Module):
    ...
    def __init__(self, version, heads, head_conv, inplanes=1280):
        super(EffNet, self).__init__()
        self.inplanes = inplanes
        self.deconv_with_bias = False
        self.heads = heads
        self.head_conv = head_conv
        self.model = EfficientNet.from_pretrained('efficientnet-b' + str(version))
    ...
    def forward(self, x):
        ...
        ret = {}
        for head in self.heads:
            ret[head] = self.__getattr__(head)(features)
        return [ret]
